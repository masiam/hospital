...
return (
    ...
    viewport={{once: true}}
     ...
     className='container mx-auto py-4 pt-20 px-6 md:px-20 lg:px-32 my-20 w-full overflow-hidden' id='Projects'>
         <h1 className='text-2xl sm:text-4x1 font-bold mb-2 text-center'>Projects <span className="underline underline-offset-4 decoration-1 under font-light">Completed</span></h1>
            ...
            style={{transform:`translateX(-${(currentIndex*100)/cardsToShow}%)`}}>
                {projectsData.map((projet,index)=>(
                    <div key={index} className='relative flex-shrink-0 w-full sm:w-1/4'>
                        <img src={projet.image} alt={projet.title} className='w-full h-auto mb-14'/>
                        <div className='absolute left-0 right-0 bottom-5 flex justify-center'>
                            ...
                            <h2 className='text-xl font-semibold text-gray-800'>
                                ...
                                <p className='text-gray-500 text-sm'>
                                    {projet.price} <span className='px-1'>|</span>{projet.location}
                                </p>
